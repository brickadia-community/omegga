...
<template>
  <div :class="['log-entry', { focused: $route.params.time == log.created }]">
    <div class="log-row">
      <router-link
        :to="
          '/history/' + ($route.params.time == log.created ? '' : log.created)
        "
        class="time-link"
      >
        <LinkIcon />
      ...
      <div v-if="log.action === 'msg'" class="chat-message message" v-once>
        ...
        ><router-link
          v-else
          :to="'/players/' + log.user.id"
          class="user"
          :style="{ color: '#' + log.user.color }"
          >{{ log.user.name }}</router-link
        >{{ log.user.web ? ']' : '' }}:
        <span v-html="xss(log.message)" v-linkified />
      </div>
